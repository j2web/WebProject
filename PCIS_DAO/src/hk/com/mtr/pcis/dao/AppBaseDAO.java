...
public class AppBaseDAO {
	...
	protected Query createQuery(String sql, Map<?, Object> parameterMap, Class<?> type, boolean isNative) {
		...
		if (parameterMap.size() != 0) {
			...
			while (it.hasNext()) {
				...
				if (key instanceof String)
					...
					log.warn("The paramter name [" + key.getClass() + "] is unknow type");
			}
	...
	protected Query createQuery(String sql, Map<?, Object> parameterMap, boolean isNative) {
		return createQuery(sql, parameterMap, null, isNative);
	}

	protected Query createPagedQuery(PageInfoVO pageInfoVO, String sql, Map<?, Object> parameterMap) {
		return createPagedQuery(pageInfoVO, sql, parameterMap, false);
	}

	protected Query createPagedQuery(PageInfoVO pageInfoVO, String sql, Map<?, Object> parameterMap, boolean isNative) {
	...
	protected void buildRecordCount(PageInfoVO pageInfoVO, String sql, Map<?, Object> parameterMap) {
		buildRecordCount(pageInfoVO, sql, parameterMap, false);
	}

	protected void buildRecordCount(PageInfoVO pageInfoVO, String sql, Map<?, Object> parameterMap, boolean isNative) {
